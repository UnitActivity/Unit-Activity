...
class EditUkmPage extends StatefulWidget {
  final Map<String, dynamic> ukmData;

  const EditUkmPage({super.key, required this.ukmData});

  @override
  State<EditUkmPage> createState() => _EditUkmPageState();
}

class _EditUkmPageState extends State<EditUkmPage> {
  ...
  void initState() {
    super.initState();
    // Pre-fill form with existing UKM data
    _nameController = TextEditingController(
      text: widget.ukmData['nama_ukm'] ?? '',
    );
    _emailController = TextEditingController(
      text: widget.ukmData['email'] ?? '',
    );
    _descriptionController = TextEditingController(
      text: widget.ukmData['description'] ?? '',
    );
    _selectedImageUrl = widget.ukmData['logo'];
  }
  ...
  Future<void> _updateUkm() async {
    ...
    try {
      ...
      final ukmId = widget.ukmData['id_ukm'].toString();

      // Check if email already exists (exclude current UKM)
      if (email != widget.ukmData['email']) {
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Edit informasi UKM di bawah ini',
                    style: GoogleFonts.inter(
                      fontSize: 14,
                    ...
                    children: [
                      ...
                      Expanded(
                        child: ElevatedButton(
                          onPressed: _isLoading ? null : _updateUkm,
